...
const CreateExpenseScreen = () => {
  const { billId, groupId } = useLocalSearchParams<{ billId: string; groupId: string }>();
  ...
  const payerMember = members?.find(m => helpers.getMemberId(m) === form.paidBy);
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent}>
          ...
          {/* Payer & Amount */}
          <PayerAmountSection 
             payerName={payerMember ? helpers.getMemberName(payerMember) : 'Chọn người'}
          ...
          <SplitList 
             ...
             getAvatar={(id) => members?.find(m => helpers.getMemberId(m) === id)?.user?.avatar || null}
          />
        ...
        <View style={styles.fixedFooter}>
           <View style={styles.totalInfoContainer}>
              ...
              <Text style={[styles.totalValueFooter, !calc.isValid && styles.totalError]}>
                  {calc.totalCalculated.toLocaleString('vi-VN')}đ
              </Text>
           </View>
           {!calc.isValid && (
                <Text style={styles.errorTextFooter}>
                   Còn thiếu: {(parseFloat(form.amount || '0') - calc.totalCalculated).toLocaleString('vi-VN')}đ
                </Text>
           )}
           <TouchableOpacity 
              style={[styles.saveButton, (!calc.isValid || isPending) && styles.saveButtonDisabled]}
              onPress={logic.submit}
              disabled={!calc.isValid || isPending}
           >
              {isPending ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveButtonLabel}>Lưu chi tiêu</Text>}
           </TouchableOpacity>
        </View>
      ...
      <SelectionModal 
        ...
        options={members?.map(m => ({ label: helpers.getMemberName(m), value: helpers.getMemberId(m) })) || []}
        onSelect={setters.setPaidBy} selectedValue={form.paidBy}
      />
    </SafeAreaView>
  );
};
