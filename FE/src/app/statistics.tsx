...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, ActivityIndicator, Dimensions, Modal, TouchableWithoutFeedback } from 'react-native';
...
import { format, subDays, parseISO, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, startOfMonth, endOfMonth, eachWeekOfInterval, isSameWeek, subWeeks, addWeeks, subMonths, addMonths, startOfDay, endOfDay, addDays } from 'date-fns';
...
const StatisticsScreen = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (viewMode === 'week') {
            ...
            dateRangeLabel = `${format(start, 'dd/MM')} - ${format(end, 'dd/MM/yyyy')}`;
        ...
        allExpenses.forEach(e => { expensesMap[e.id] = e; });
        ...
        const paidExpenses = allExpenses.filter(e => e.paidBy === String(userId));
        ...
        if (statType === 'share') {
            list = filteredShares.map(s => ({
                ...s,
                expense: expensesMap[s.expenseId]
            })).sort((a, b) => new Date(b.expense.createdTime).getTime() - new Date(a.expense.createdTime).getTime());
            ...
            if (viewMode === 'week') {
                ...
                const startLoop = startOfWeek(start, { locale: vi, weekStartsOn: 1 });
                const endLoop = endOfWeek(end, { locale: vi, weekStartsOn: 1 });
                const weeks = eachWeekOfInterval({ start: startLoop, end: endLoop }, { locale: vi, weekStartsOn: 1 });
                ...
                chartData = weeks.map((weekStart, index) => {
                    ...
                    if (actualStart > actualEnd) return { label: format(weekStart, 'dd/MM'), value: 0 };
                ...
                chartData = [{ label: format(currentDate, 'EE', { locale: vi }), value: shareTotal }];
            }
        } else {
            // Paid Logic
            list = filteredPaid.map(e => ({
                id: e.id,
                expense: e,
            ...
            if (viewMode === 'week') {
                const days = eachDayOfInterval({ start, end });
                chartData = days.map(day => {
                    const dayExpenses = paidExpenses.filter(e => isSameDay(parseISO(e.createdTime), day));
                    return {
                        label: format(day, 'EE', { locale: vi }),
                ...
                chartData = weeks.map((weekStart, index) => {
                 ...
                 chartData = [{ label: format(currentDate, 'EE', { locale: vi }), value: paidTotal }];
            }
        }
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{ title: 'Thống kê chi tiêu', headerBackTitle: 'Trở về' }} />
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.cardsContainer}>
            <TouchableOpacity 
                style={[styles.card, statType === 'share' ? styles.activeCard : styles.inactiveCard]}
                onPress={() => setStatType('share')}
                activeOpacity={0.9}
            >
                <View style={styles.cardHeader}>
                    <Ionicons name="wallet-outline" size={20} color={statType === 'share' ? 'white' : '#666'} />
                    <Text style={[styles.cardTitle, statType === 'share' ? {color:'white'} : {color:'#666'}]}>Thực chi</Text>
                </View>
                <Text style={[styles.cardValue, statType === 'share' ? {color:'white'} : {color:'#333'}]}>
            ...
            <TouchableOpacity 
                style={[styles.card, statType === 'paid' ? styles.activeCard : styles.inactiveCard]}
                onPress={() => setStatType('paid')}
                activeOpacity={0.9}
            >
                <View style={styles.cardHeader}>
                    <Ionicons name="cash-outline" size={20} color={statType === 'paid' ? 'white' : '#666'} />
                    <Text style={[styles.cardTitle, statType === 'paid' ? {color:'white'} : {color:'#666'}]}>Đã trả</Text>
                </View>
                <Text style={[styles.cardValue, statType === 'paid' ? {color:'white'} : {color:'#333'}]}>
        ...
        {loading ? (
            <ActivityIndicator size="large" color={APP_COLOR.ORANGE} style={{ marginTop: 50 }} />
        ) : (
            <>
                {/* Chart Section */}
                <View style={styles.chartSection}>
                    ...
                    <View style={{ flexDirection: 'row', height: CHART_HEIGHT, alignItems: 'flex-end' }}>
                        {/* Y-Axis */}
                        <View style={{ justifyContent: 'space-between', height: '100%', paddingBottom: X_AXIS_HEIGHT, paddingRight: 10, width: 40 }}>
                            {yAxisLabels.map((label, i) => (
                                <Text key={i} style={{ fontSize: 10, color: '#999', textAlign: 'right' }}>{label}</Text>
                            ))}
                        </View>
                        ...
                        <View style={{ flex: 1, height: '100%' }}>
                            {/* Grid Lines */}
                            <View style={{ ...StyleSheet.absoluteFillObject, justifyContent: 'space-between', paddingBottom: X_AXIS_HEIGHT }}>
                                {yAxisLabels.map((_, i) => (
                                    <View key={i} style={{ borderTopWidth: 1, borderColor: '#eee', borderStyle: 'dashed', width: '100%', height: 1 }} />
                                ))}
                            </View>

                            {/* Bars */}
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', flex: 1, paddingBottom: 0 }}>
                                {displayData.chart.map((d, i) => {
                                    const barHeight = (d.value / maxChartValue) * PLOT_HEIGHT;
                                    const isSelected = selectedPoint?.label === d.label;
                                    const isMax = d.value === Math.max(...displayData.chart.map(x => x.value)) && d.value > 0;
                                    ...
                                    return (
                                        ...
                                        >
                                            <View style={{ 
                                                ...
                                                backgroundColor: isSelected || isMax ? APP_COLOR.ORANGE : '#FFCCBC',
                                                ...
                                                {isSelected && (
                                                    <View style={styles.tooltip}>
                                                        <Text style={styles.tooltipText}>{d.value.toLocaleString('vi-VN')}</Text>
                                                    </View>
                                                )}
                                            </View>
                                            <View style={{ height: X_AXIS_HEIGHT, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                    ...
                    {displayData.list.length > 0 ? (
                        displayData.list.map((item) => (
                            <TouchableOpacity 
                                ...
                                onPress={() => router.push(`/(tabs)/groups/expense/${item.expense.id}`)}
                            >
                                <View style={styles.itemIconBg}>
                                    <Ionicons name="cart-outline" size={24} color={APP_COLOR.ORANGE} />
                                </View>
                                <View style={styles.itemContent}>
                                    <Text style={styles.itemName} numberOfLines={1}>{item.expense.description}</Text>
                                    <Text style={styles.itemSub}>
                                        {format(parseISO(item.expense.createdTime), 'dd/MM/yyyy')}
                                    </Text>
                                </View>
                                <Text style={styles.itemAmount}>-{item.shareAmount.toLocaleString('vi-VN')} đ</Text>
                            </TouchableOpacity>
                        ))
                    ) : (
      ...
      {showDropdown && (
        <>
            ...
            <View style={[styles.dropdownMenu, { position: 'absolute', top: 60, left: 20, zIndex: 100 }]}>
                {(['day', 'week', 'month'] as const).map((mode) => (
                    <TouchableOpacity 
                        key={mode}
                        style={[styles.dropdownItem, viewMode === mode && styles.activeDropdownItem]}
                        onPress={() => {
                            setViewMode(mode);
                    ...
                    >
                        <Text style={[styles.dropdownItemText, viewMode === mode && styles.activeDropdownItemText]}>
                            {viewModeLabels[mode]}
                        </Text>
                        {viewMode === mode && <Ionicons name="checkmark" size={18} color={APP_COLOR.ORANGE} />}
                    </TouchableOpacity>
                ))}
            </View>
        </>
